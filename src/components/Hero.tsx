...
const Hero = () => {
  ...
  return <div className="relative w-full h-screen overflow-hidden">
      {carouselItems.map((item, index) => <div key={item.id} className={`carousel-item absolute inset-0 transition-opacity duration-1000 ${index === currentSlide ? 'opacity-100' : 'opacity-0'}`} style={{
      backgroundImage: `url(${item.image})`
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center text-white p-4">
            {item.showLogo && <div className="mb-8 animate-fade-in">
                <img src="https://github.com/adeniji4/Mio-hospitality/blob/main/public/Photos/b4196b07-03dc-40ee-a6b4-34e891c1b9bd.png" alt="Mio's Hospitality & C0" className="h-64 w-auto object-contain filter brightness-125" />
              </div>}
